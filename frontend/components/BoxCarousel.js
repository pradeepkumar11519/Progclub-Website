...
const CarouselCard = ({ title, subtitle, image, description, key }) => (
    <div key={key} className="boxcarouselcard xl:w-10/12 mx-auto">
        <h2 className="lg:text-3xl text-stone-800 lg:pb-3 m-0 pb-2 pt-3">
            {title}
        </h2>
        <Image
            alt="Upcoming Events"
            ...
            height={400}
            width={400}
            className="w-full  xl:h-[320px] lg:h-[280px] mx-auto rounded-2xl"
            src={image}
        />
        <p className="text-sm lg:text-xl text-center my-3 text-gray-900 font-medium">
            {subtitle}
        </p>
        <p className="text-xs lg:text-sm text-center text-gray-700">
            {description}
        </p>
    </div>
);
